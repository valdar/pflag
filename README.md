...
`pflag`, by default, does not parse the shorthand versions of go test's built-in flags (i.e., those starting with `-test.`).
...
func main() {
	flag.CommandLine.AddGoFlagSet(goflag.CommandLine)
    flag.CommandLine.ParseSkippedFlags(os.Args[1:], goflag.CommandLine)
	flag.Parse()
}
```

### Configure prefixes of shorthand flags that must be skipped during parsing
`pflag` allow you to configure prefixes of shorthand flags to be skipped during parsing (default is to skip shorthand flags starting with `test.`)

**Example**: You want to also skip ginkgo shorthand flags
```go
flags.SetShortHandSkipPrefixes([]string{"test.","ginkgo."})
```
